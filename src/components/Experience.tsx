    ...
    >
      ...
      <div className="relative max-w-[1800px] mx-auto px-6 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          {/* Right Content - Stats */}
          <div
            className={`transition-all duration-1000 delay-300 ${isInView ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'
              }`}
          >
            <div className="grid grid-cols-2 gap-8 lg:gap-12">
              {stats.map((stat, index) => (
                <div
                  key={stat.label}
                  className={`text-center p-8 border border-gotam-cream/10 hover:border-gotam-gold/30 transition-all duration-500 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                    }`}
            ...
            {/* Additional Info */}
            <div
              className={`mt-12 p-8 bg-gotam-charcoal/50 backdrop-blur-sm border border-gotam-cream/5 transition-all duration-1000 delay-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                }`}
            >
